...
import com.mybucketlistbook.service.UserService;
...
public class UserController {
    ...
    public ResponseEntity signup(@Valid @RequestBody SignUpDto signUpDto) throws UnknownHostException, MessagingException {
    ...
    @DeleteMapping("/user")
    @ApiOperation(value = "회원 탈퇴", notes = "<strong>token 을 통해 </strong> 해당 유저를 탈퇴시킨다.")
    @ApiResponses({
    ...
    @ApiOperation(value = "현재 비밀번호 확인", notes = "<strong>token 과 현재 비밀번호</strong>를 이용해 해당 유저의 현재 비밀번호가 맞는지 확인한다.")
    @ApiResponses({
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<Map<String, Boolean>> checkPassword(@RequestBody @Valid PasswordDto passwordDto) {
        return ResponseEntity.ok(userService.checkPassword(passwordDto));
    }

    @PutMapping("/user/password")
    @ApiOperation(value = "회원 비밀번호 변경", notes = "<strong>token 과 변경할 비밀번호</strong>를 이용해 해당 유저의 비밀번호를 변경한다.")
    @ApiResponses({
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity changeUserPassword(@RequestBody @Valid ChangePasswordDto changePasswordDto) {
    ...
    @ApiOperation(value = "현재 닉네임 변경", notes = "<strong>token 과 변경할 닉네임 </strong>를 이용해 해당 유저의 닉네임을 변경한다.")
    @ApiResponses({
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity changeUserNickname(@RequestBody @Valid ChangeNicknameDto changeNicknameDto) {
        userService.updateNickname(changeNicknameDto);

        return new ResponseEntity(HttpStatus.OK);
    }

    @PutMapping("/user/image")
    @ApiOperation(value = "유저 프로필 이미지 변경", notes = "<strong>token 과 선택된 imgNum</strong>를 이용해 해당 유저의 프로필 이미지를 변경한다.")
    @ApiResponses({
